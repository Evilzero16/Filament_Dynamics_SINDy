...
model = ps.SINDy(feature_library=library,
                 optimizer=ps.STLSQ(threshold=1e-11, alpha=1e-3))
model.fit(X, t=dt, x_dot=dL_dt.reshape(-1, 1))

pred = model.simulate([L_avg_smooth[0]], t_uniform)
print("\nSINDy model:")
model.print()
print(f"MSE = {mean_squared_error(L_avg_smooth, pred)}")
c0, c2 = model.coefficients().flatten()
print(f"c0={c0:.3g}, c2={c2:.3g}")

plt.figure()
plt.plot(t_uniform, L_avg, 'b', label='⟨L_avg⟩ raw')
plt.plot(t_uniform, L_avg_smooth, 'r', label='smoothed')
plt.plot(t_uniform, pred, 'k--', label='SINDy sim')
plt.legend(); plt.xlabel('t'); plt.ylabel('⟨L_avg⟩')

plt.figure()
plt.plot(t_uniform, dL_dt, 'b', label='finite diff')
plt.plot(t_uniform, model.predict(X), 'r--', label='SINDy')
plt.legend(); plt.xlabel('t'); plt.ylabel('d⟨L_avg⟩/dt')
plt.show()
